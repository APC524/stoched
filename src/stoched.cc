...
int main(int argc, char *argv[]) {
  ...
  int n_vars = model->getVarsCount();
  int n_events = model->getEventsCount();
  ...
  if(inits_v.empty()){
    fprintf(stderr,
            "Warning: no initial values specified. Initializing "
            "all variables as equal to zero \n");
